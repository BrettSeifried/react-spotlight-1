...
// export const printAddress = (user) => {
export const printAddress = ({ name, address: { street, number, city, country } }) => {
  // return 'Bob Smith lives at 123 Main Street, Anytown, USA'
  //   return `${user.name} lives at ${user.address.number}, ${user.address.street} ${user.address.city}, ${user.address.country}`;
...
export const totalSneakerCount = (data) => {
  return Object.keys(data).reduce((acc, key) => (acc += data[key].shoes.length), 0);
};
...
export const convertToArray = (arr) => {
  return Object.keys(arr).map((key) => [key, arr.key]);
};
